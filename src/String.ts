...
export function asciiDecode(bytes: Uint8Array) {
  ...
  for (const byte of bytes) {
    console.assert(isASCIIByte(byte), "asciiDecode requires an ASCII byte sequence.")
  }
...
export function collectASequenceOfCodePoints(condition: ((str: string) => boolean),
   ...
   * 3. Return result.
   */
  if (!isArray(input)) return collectASequenceOfCodePoints(condition, Array.from(input), options)
...
export function skipASCIIWhitespace(input: string | string[], options: { position: number }) {
   ...
   * is still updated.
   */
  collectASequenceOfCodePoints(str => ASCIIWhiteSpace.test(str), input, options)
}
...
export function strictlySplit(input: string | string[], delimiter: string) {
   ...
   * 6. Return tokens.
   */
  if (!isArray(input)) return strictlySplit(Array.from(input), delimiter)
...
export function splitAStringOnASCIIWhitespace(input: string | string[]) {
   ...
   * 5. Return tokens.
   */
  if (!isArray(input)) return splitAStringOnASCIIWhitespace(Array.from(input))

  const options = { position: 0 }
  const tokens: string[] = []
  skipASCIIWhitespace(input, options)
  while (options.position < input.length) {
    const token = collectASequenceOfCodePoints(str => !ASCIIWhiteSpace.test(str), input, options)
    tokens.push(token)
    skipASCIIWhitespace(input, options)
  }
  return tokens
...
export function splitAStringOnCommas(input: string | string[]) {
   ...
   * 4. Return tokens.
   */
  if (!isArray(input)) return splitAStringOnCommas(Array.from(input))
